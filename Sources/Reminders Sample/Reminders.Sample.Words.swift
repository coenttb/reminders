...
extension Reminders.Sample.Words {
    ...
    static func title(_ random: inout Reminders.Sample.Random) -> String {
        "\(verbs[random.next(in: 0..<verbs.count)]) \(objects[random.next(in: 0..<objects.count)])"
    }

    static func note(_ random: inout Reminders.Sample.Random) -> String { notes[random.next(in: 0..<notes.count)] }
}
