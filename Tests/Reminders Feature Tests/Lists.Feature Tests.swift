...
struct `Lists feature` {
    ...
    func makeStore(
        ...
        restoring: @escaping @Sendable (inout Lists.Feature.State.DebugSnapshot) -> Void = { _ in }
    ) async throws -> TestStoreActor<Lists.Feature> {
        try await withDependencies { $0.continuousClock = clock } operation: {
            try await TestStoreActor(initialState: Lists.Feature.State()) { Lists.Feature() } changes: { [today] in
                $0.today = today
                restoring(&$0)
            }
        }
