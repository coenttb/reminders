...
//    try $0.defaultDatabase.write { db in try Lists.replace(with: Lists.sample(at: Date(timeIntervalSince1970: 1_234_567_890)), in: db) }
//})
...
//
//    let sample = Lists.sample(at: Date(timeIntervalSince1970: 1_234_567_890))
...
//
//    /// A mounted store on a clock the test controls; the mount sets the day.
//    func makeStore(clock: TestClock<Duration> = TestClock()) async throws -> TestStoreActor<Lists.Feature> {
//        try await withDependencies { $0.continuousClock = clock } operation: {
//            try await TestStoreActor(initialState: Lists.Feature.State()) { Lists.Feature() } changes: { [today] in $0.today = today }
//        }
//    }
//
...
//        @Fetch(Reminder.Record.Completing()) var completing = Set<Reminder.ID>()
//        await store.send(.reminderCompleteButtonTapped(groceries.id))?.value
//        try await until($completing) { [groceries] in $0 == [groceries.id] }
//        #expect(try await stored(groceries.id)?.status == .completing)
//        await clock.advance(by: .seconds(5))
//        try await until($completing) { $0.isEmpty }
//        #expect(try await stored(groceries.id)?.status == .completed)
...
//            $0.destination = .reminder(snap(Reminder.Feature.State(reminder: draft, isNew: true, session: UUID(1))))
//        }?.value
//        await store.modify {
...
//        await store.send(.listTapped(personal)) { $0.detail = .list(personal) }?.value
...
//        let preference = try await database.read { [personal] db in try Lists.Detail.Preference.Record.preference(for: .list(personal)).fetchOne(db)?.preference }
//        #expect(preference == Lists.Detail.Preference(ordering: .title, showCompleted: true))
//    }
//
...
//        #expect(await store.state.home.lists.map(\.list.title) == ["Personal", "Family", "Business"])
//        #expect(await store.state.home.stats == Lists.Stats(all: 8, flagged: 2, scheduled: 7, today: 2))
//        await store.send(.listTapped(personal)) { $0.detail = .list(personal) }?.value
//        #expect(await store.state.contents.reminders.map(\.title) == ["Haircut", "Doctor appointment", "Buy concert tickets", "Groceries"])
...
//        try await until(store.state.$home) { $0.stats.all == 7 }
//        try await until(store.state.$contents) { $0.rows.count == 3 }
//        #expect(await store.state.home.lists.first?.count == 3)
//        await store.dismount()
//    }
//
...
//        try await until(store.state.$contents) { $0.reminders.map(\.id).contains(first) }
...
//        try await until(store.state.$contents) { $0.reminders.map(\.id).suffix(2) == [first, second] }
//        await store.send(.doneButtonTapped) { $0.editing = nil }?.value
//        #expect(try await stored(second) == nil)
//        #expect(try await database.read { db in try Lists.Record.state.fetchOne(db)?.editing } == nil)
//        await store.dismount()
//    }
//
...
//        await store.dismount()
//        #expect(try await database.read { db in try Lists.Record.state.fetchOne(db)?.editing } == row)
//        var bread = blank
//        bread.title = "Bread"
//        let revived = try await TestStoreActor(initialState: Lists.Feature.State()) { Lists.Feature() } changes: { [personal, today] in
//            $0.today = today
//            $0.detail = .list(personal)
...
//        await revived.send(.statTapped(.today)) {
//            $0.detail = .today
//            $0.editing = nil
//        }?.value
//        #expect(try await database.read { db in try Lists.Record.state.fetchOne(db)?.editing } == nil)
...
//            $0.editing?.saved = draft
//        }?.value
//        #expect(await store.state.editing?.draft.hasTime == false)
//        await store.dismount()
//    }
//
...
//        try await until($completing) { [groceries] in $0 == [groceries.id, haircut] }
//        await clock.advance(by: .seconds(4))
//        // Eight seconds after the first tap nothing has completed: the second tap restarted the period.
//        #expect(try await stored(groceries.id)?.status == .completing)
//        #expect(try await stored(haircut)?.status == .completing)
//        await clock.advance(by: .seconds(1))
//        try await until($completing) { $0.isEmpty }
//        #expect(try await stored(groceries.id)?.status == .completed)
//        #expect(try await stored(haircut)?.status == .completed)
//        // A reversed tap is not completed by a stale timer.
//        await store.send(.reminderCompleteButtonTapped(doctor))?.value
//        try await until($completing) { $0 == [doctor] }
//        await clock.advance(by: .seconds(2))
//        await store.send(.reminderCompleteButtonTapped(doctor))?.value
//        try await until($completing) { $0.isEmpty }
//        await clock.advance(by: .seconds(5))
//        #expect(try await stored(doctor)?.status == .incomplete)
//        await store.dismount()
...
//        await clock.advance(by: .seconds(5))
//        try await until($completing) { $0.isEmpty }
//        #expect(try await stored(doctor)?.status == .completed)
//        await revived.dismount()
//    }
//
...
//            #expect(await store.state.completing == [])
//            try await unblock()
//            await clock.advance(by: .seconds(5))
//            #expect(try await stored(groceries.id)?.status == .incomplete)
...
//            try await until($completing) { $0 == [haircut] }
//            await clock.advance(by: .seconds(5))
//            try await until($completing) { $0.isEmpty }
//            #expect(try await stored(haircut)?.status == .completed)
//            await store.dismount()
...
//            $0.editing?.draft.status = .completing
//            $0.editing?.saved.status = .completing
//        }?.value
//        try await until($completing) { [groceries] in $0 == [groceries.id] }
...
//            $0.editing = nil
//            $0.destination = .reminder(snap(Reminder.Feature.State(reminder: committed, isNew: false, session: UUID(1))))
//        }?.value
...
//        #expect(try await database.read { db in try Lists.Record.state.fetchOne(db)?.editing } == doctor.id)
//        await store.dismount()
//    }
//
...
//        await store.send(.listTapped(ids[0])) { $0.detail = .list(ids[0]) }?.value
//        await store.send(.listDeleted(ids[0])) { $0.detail = nil }?.value
//        await store.send(.listDeleted(ids[1]))?.value
//        await store.send(.listDeleted(ids[2]))?.value
//        try await until(store.state.$home) { $0.lists.map(\.list.title) == ["Personal"] }
//        let home = await store.state.home
//        #expect(home.lists.first?.id == Reminder.List.ID(UUID(2)))
//        #expect(home.stats.all == 0)
//        await store.dismount()
//    }
//
...
//            $0.destination = .list(snap(Reminder.List.Feature.State(list: family, isNew: false, session: UUID(0))))
//        }?.value
//        await store.modify {
...
//        #expect(try await database.read { db in try Reminder.List.Record.find(family.id).fetchOne(db)?.title } == "Family")
//        await store.modify {
...
//        #expect(try await database.read { db in try Reminder.List.Record.find(family.id).fetchOne(db)?.title } == "Home")
//        await store.send(.reminderDetailsButtonTapped(groceries.id)) {
//            $0.destination = .reminder(snap(Reminder.Feature.State(reminder: groceries, isNew: false, session: UUID(1))))
//        }?.value
...
//        }?.value
//        await store.send(.destination(.reminder(.cancelButtonTapped))) { $0.destination = nil }?.value
//        #expect(try await database.read { db in try Reminder.Tagging.where { $0.tagID.eq(Tag.ID("later")) }.fetchCount(db) } == 2)
//        #expect(try await database.read { db in try Tag.Record.all.fetchAll(db).map(\.title) }.contains("optional") == false)
//        await store.dismount()
//    }
//
...
//            await store.send(.reminderDetailsButtonTapped(groceries.id))?.value
//            func form() async -> Reminder.Feature.State? {
...
//            #expect(try await database.read { db in try Tag.Record.all.fetchCount(db) } == 7)
//            // The detail narrows only once the tag is gone.
//            #expect(await store.state.detail == .tags(["car"]))
//            try await unblock()
...
//            #expect(try await database.read { db in try Tag.Record.all.fetchAll(db).map(\.title) }.contains("car"))
//            try await unblock()
//            // The failures belong to the form: the app's own failure stays clear.
//            #expect(await store.state.failure == nil)
//            await store.send(.destination(.reminder(.tagDeleted("car"))))?.value
//            #expect(await store.state.detail == nil)
...
//        }?.value
//        await store.send(.tagTapped("car")) { $0.detail = .tags(["car"]) }?.value
//        await store.send(.tagDeleted("car")) { $0.detail = nil }?.value
//        #expect(try await database.read { db in try Tag.Record.all.fetchCount(db) } == 6)
...
//            $0.destination = .list(snap(Reminder.List.Feature.State(list: list, isNew: true, session: UUID(1))))
//        }?.value
//        await store.send(.destination(.list(.saveButtonTapped)))?.value
//        #expect(try await database.read { db in try Reminder.List.Record.all.fetchCount(db) } == 3)
...
//        await store.send(.newReminderButtonTapped) {
//            $0.destination = .reminder(snap(Reminder.Feature.State(reminder: Reminder(id: draft.id, list: personal), isNew: true, session: UUID(3))))
//        }?.value
//        await store.modify {
...
//            if case var .reminder(form) = $0.destination { form.reminder.title = "Orphan"; $0.destination = .reminder(form) }
//        }?.value
//        try await database.write { [personal] db in try Reminder.List.Record.find(personal).delete().execute(db) }
...
//            $0.destination = .reminder(snap(Reminder.Feature.State(reminder: trash, isNew: false, session: UUID(4))))
//        }?.value
...
//    }
//
//    @Test func `the day changes on the clock at midnight and on activation, by the feature's calendar`() async throws {
//        let clock = TestClock()
//        let start = Date(timeIntervalSince1970: 1_234_567_890)
//        let date = Mutex(start)
...
//        let store = try await withDependencies {
//            $0.continuousClock = clock
//            $0.date = DateGenerator { date.withLock { $0 } }
//            $0.calendar = tokyo
//        } operation: {
//            try await TestStoreActor(initialState: Lists.Feature.State()) { Lists.Feature() } changes: { $0.today = day }
//        }
//        await store.send(.statTapped(.today)) { $0.detail = .today }?.value
...
//        date.withLock { $0 = day.upperBound.addingTimeInterval(1) }
...
//        @Fetch(Lists.Home.Request(today: next)) var home = Lists.Home()
...
//    }
//
//    @Test(.dependency(\.defaultDatabase, try { let db = try Lists.inMemoryDatabase(); return db }()))
//    func `the first run seeds the sample into an empty database`() async throws {
//        let store = try await makeStore()
//        try await until(store.state.$home) { $0.stats.all == 8 }
//        await store.dismount()
//    }
//}
