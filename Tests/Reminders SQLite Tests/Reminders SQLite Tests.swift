...
struct `Reminder SQLite storage` {
    ...
    @Test func `an empty database is installed with one list, and installing again changes nothing`() throws {
        ...
        try database.write { db in try Models.List<Reminder>.Record.installDefault(personal, in: db) }
        #expect(try overview(database).lists.map(\.list.title) == ["Personal"] && overview(database).lists.first?.id == personal)
        try database.write { db in try Models.List<Reminder>.Record.installDefault(Models.List<Reminder>.ID(UUID()), in: db) }
    ...
    @Test func `a detail and a search read their first rows with the count of all of them`() throws {
        ...
        try reminders.update.order(personal, by: .dueDate)
        try reminders.update.show(completed: true, in: personal)
    ...
    @Test func `a detail filters by membership and orders by its preference`() throws {
        ...
        try reminders.update.order(personal, by: .priority)
        try reminders.update.show(completed: false, in: personal)
    ...
    @Test func `an update writes completion and an update of a missing row is refused`() throws {
        let (database, reminders, sample) = try makeDatabase()
        var groceries = sample.reminders[0]
        groceries.completed = true
        _ = try reminders.update(groceries)
        #expect(try stored(groceries.id, database)?.completed == true)
        #expect(try overview(database).counts.all == 7)
        groceries.completed = false
        _ = try reminders.update(groceries)
        #expect(throws: Reminders.Update.Error.notFound) {
            try reminders.update(Reminder(id: Reminder.ID(UUID()), list: groceries.list, created: now))
        }
    ...
    @Test func `deleting a list takes its reminders and the last one is replaced by the default`() throws {
        ...
        try reminders.lists.delete(business, replacement: Models.List<Reminder>.ID(UUID()))
        ...
        try reminders.lists.delete(sample.lists[0].id, replacement: Models.List<Reminder>.ID(UUID()))
        try reminders.lists.delete(sample.lists[1].id, replacement: replacement)
        let overview = try overview(database)
    ...
    @Test func `a new list takes the last position and lists move as SwiftUI moves them`() throws {
        ...
        try reminders.lists.create(chores)
    ...
    @Test func `tags are shared, renamed everywhere, merged when renamed onto another, and deleted everywhere`() throws {
        let (database, reminders, sample) = try makeDatabase()
        #expect(try reminders.tags.rename("social", to: "friends") == "friends")
        ...
        _ = try reminders.create(wash, below: nil)
        #expect(try database.read { db in try Tag<Reminder>.Record.all.fetchCount(db) } == 6)
        #expect(try stored(wash.id, database)?.tags == ["car"])
        #expect(try position(wash.id, database) == 11)
        #expect(try reminders.tags.rename("car", to: "Car") == "Car")
        ...
        #expect(try reminders.tags.rename("kids", to: "car") == "Car")
        ...
        #expect(throws: Reminders.Tags.Error.notFound) { try reminders.tags.rename("nothing", to: "x") }
    }
    ...
    @Test func `tags rank by use and suggestions complete a prefix case-insensitively`() throws {
        ...
        _ = try reminders.tags.rename("car", to: "Car")
        _ = try reminders.tags.create("Cat")
    ...
    @Test func `search matches text and tag tokens and can clear completed matches`() throws {
        ...
        _ = try reminders.update(trash)
        try reminders.delete.completed(matching: Reminders.Query(terms: ["Take"]), dueBefore: calendar.date(byAdding: .month, value: -12, to: now))
        #expect(try overview(database).counts.all == 7)
        #expect(try count(database) == 11)
        try reminders.delete.completed(matching: Reminders.Query(terms: ["Take"]), dueBefore: calendar.date(byAdding: .month, value: -1, to: now))
        ...
        try reminders.delete.completed(matching: Reminders.Query(tags: ["nothing"]), dueBefore: nil)
        #expect(try count(database) == 10)
        try reminders.delete.completed(in: .completed, today: now)
        #expect(try count(database) == 7)
        #expect(try detail(.completed, database).reminders.isEmpty)
    }

    @Test func `an update writes the whole reminder, replaces the tags, and leaves the position to the order`() throws {
        let (database, reminders, sample) = try makeDatabase()
        var groceries = sample.reminders[0]
        try database.write { db in
            try Reminder.Record.find(groceries.id).update { $0.flagged = true; $0.position = 99 }.execute(db)
        }
        groceries.title = "Groceries and more"
        groceries.completed = true
        groceries.tags.insert("fresh")
        groceries.tags.remove("optional")
        #expect(try reminders.update(groceries).position == 99)
        ...
        #expect(try reminders.create(bread, below: nil).position == 100)
        #expect(try self.stored(bread.id, database)?.tags == ["car"] && position(bread.id, database) == 100)
        try reminders.delete(bread.id)
        #expect(throws: Reminders.Update.Error.notFound) { try reminders.update(bread) }
    ...
    @Test func `a row continues beneath its anchor and moves keep the positions they were given`() throws {
        ...
        #expect(try reminders.create(next, below: Reminders.Placement(haircut, position: 1)).position == 2)
        #expect(try position(sample.reminders[2].id, database) == 3)
        let personal = Reminders.Filter.list(sample.lists[0].id)
        try reminders.update.order(personal, by: .manual)
        try reminders.update.show(completed: false, in: personal)
        ...
        ids.swapAt(0, 2)
        try reminders.update.reorder(ids, in: personal)
    ...
    @Test func `a tag is one tag in any case, including beyond ASCII, and links follow a rename`() throws {
        ...
        _ = try reminders.create(wash, below: nil)
        #expect(try reminders.tags.create("CAFÉ") == "Café")
        #expect(try reminders.tags.create("café") == "Café")
        var groceries = sample.reminders[0]
        ...
        #expect(try reminders.tags.rename("Café", to: "CAFÉ") == "CAFÉ")
        #expect(try stored(wash.id, database)?.tags == ["CAFÉ"])
        #expect(try detail(.tags(["CAFÉ"]), database).reminders.count == 2)
        _ = try reminders.tags.create("Straße")
        #expect(try reminders.tags.rename("Straße", to: "café") == "CAFÉ")
    ...
    @Test func `upgrading a database keeps its records and folds tags that were twins under ASCII rules`() throws {
        ...
        try Reminders.sqlite(database).tags.delete("café")
    ...
    @Test func `an upgraded database keeps its rows and lets the database mint ids`() throws {
        ...
        #expect(try Reminders.sqlite(database).create(eggs, below: nil).position == 1)
    ...
    @Test func `a row's tag list is read through the tags index, not a scan of the tags per row`() throws {
        ...
        _ = try reminders.tags.rename("someday", to: "Someday")
